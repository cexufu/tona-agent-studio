...
function memoryId(layer, text) {
  return crypto.createHash("sha256").update(layer + ":" + String(text || "").trim()).digest("hex").slice(0, 18);
}

function addMemory(db, layer, text, metadata = {}) {
  const content = String(text || "").trim().slice(0, 1200);
  if (!content || !LIMITS[layer]) return false;
  const memory = memoryState(db); const id = memoryId(layer, content); const now = new Date().toISOString();
...
function rememberHumanMessage(db, message) {
  ...
  const metadata = { chatId: message.chatId || "", senderId: message.senderId || "", sourceMessageId: message.messageId || "" };
  let changed = false;
...
function taskMemory(db, chatId) {
  const assistant = (db.settings?.assistantTasks || []).filter((item) => !chatId || item.chatId === chatId).slice(-8);
  const collaboration = (db.settings?.collaborationTasks || []).filter((item) => !chatId || item.chatId === chatId).slice(-5);
  return [
...
function relevantEntries(entries, message, limit) {
...
function fiveLayerMemoryContext(db, message, shortTermContext = "") {
  const memory = memoryState(db);
  const sections = [
    ["Core memory (stable user preferences and operating rules)", relevantEntries(memory.core, message, 6)],
    ["Semantic memory (confirmed facts and decisions)", relevantEntries(memory.semantic, message, 8)],
    ["Task memory (active and recent work state)", taskMemory(db, message?.chatId)],
    ["Episodic memory (notable prior outcomes)", relevantEntries(memory.episodic, message, 6)],
    ["Short-term memory (recent conversation)", shortTermContext ? [shortTermContext] : []]
  ];
