...
function buildIsolatedExecutorTools(env = process.env) {
  ...
  const executePolicy = { timeoutMs: 70000, retries: 0, idempotent: false, rateLimit: { maxCalls: 10, windowMs: 60000 } };
  ...
  return [
    ...
    },
    {
      ...
      description: "Run a bounded browser workflow in a remote disposable browser with an allowlisted network policy.", policy: executePolicy,
    ...
    },
    {
      ...
      description: "Call one allowlisted MCP server and tool by configured server ID; arbitrary MCP URLs are never accepted from prompts.", policy: executePolicy,
