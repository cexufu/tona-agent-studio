...
const BUILT_IN_SKILL_RECIPES = {
  skill_creator:{templateVersion:2,inputGuide:'State the recurring job, target user, input material, desired deliverable, and actions that must require confirmation.',outputContract:'A reusable Skill blueprint: trigger examples, input contract, 2-4 steps, output contract, quality checks, and safety boundaries.',qualityChecklist:['Uses only configured agents','Separates automatic analysis from confirmed actions','Has a concrete final deliverable'],steps:[{agentId:'daily_assistant',task:'Act as the product owner. Convert the request into a reusable Skill brief: user trigger, required input, desired outcome, exclusions, and approval boundaries. Ask no questions; record missing assumptions explicitly.'},{agentId:'coding_assistant',task:'Act as workflow reviewer. Turn the brief into 2-4 executable steps using only configured roles. Identify ambiguity, unsafe external actions, missing inputs, and failure cases. Return a corrected workflow specification.'},{agentId:'daily_assistant',task:'Deliver a final Skill blueprint with: name, purpose, trigger examples, input contract, numbered workflow steps, output contract, quality checklist, and a short first test case. Make it ready to paste into Skill Center.'}]},
  skill_test:{templateVersion:2,inputGuide:'Provide the Skill name or configuration, one representative input, and any expected outcome or risk.',outputContract:'A test report with cases, observed or expected behavior, failures, and a release decision.',qualityChecklist:['Covers normal, missing-input, and edge cases','Checks unsupported claims and unsafe actions','Ends with a ship, revise, or block decision'],steps:[{agentId:'coding_assistant',task:'Design a compact test matrix: happy path, incomplete input, ambiguous instruction, boundary case, and unsafe-action case. State expected behavior and pass criteria for each.'},{agentId:'research_assistant',task:'Audit the Skill output requirements for unsupported claims, evidence gaps, hallucination risk, and misleading confidence. List only material quality failures.'},{agentId:'daily_assistant',task:'Deliver a release report with tested scenarios, pass/fail findings, prioritized fixes, and a clear decision: ready, revise, or block. Do not claim a live test occurred unless evidence is present.'}]},
  feishu_learning:{templateVersion:2,inputGuide:'Paste a user-authorized Feishu document excerpt, meeting record, or group material. Include the purpose of learning it.',outputContract:'A reusable knowledge brief with traceable facts, decisions, open questions, and next actions.',qualityChecklist:['Uses only supplied material','Separates facts, decisions, and inference','Preserves uncertainty and source limits'],steps:[{agentId:'research_assistant',task:'Extract a source-grounded fact table: key claims, decisions, owners, dates, evidence, and unknowns. Do not infer missing facts or access material not explicitly supplied.'},{agentId:'daily_assistant',task:'Convert the extracted material into a reusable knowledge brief: context, confirmed facts, decisions, pending questions, action list, and suggested tags. Preserve source boundaries.'},{agentId:'daily_assistant',task:'Perform a final traceability check. Deliver a concise Chinese knowledge note with sections for facts, decisions, open items, and recommended next step; label any inference clearly.'}]},
  research_brief:{templateVersion:2,inputGuide:'Provide a paper, abstract, research news item, project call, or structured notes, plus the research question or decision it should inform.',outputContract:'A decision-ready research brief with evidence, limitations, novelty, and next research actions.',qualityChecklist:['No fabricated citations or results','Distinguishes evidence from inference','Names limitations and a falsifiable next step'],steps:[{agentId:'research_assistant',task:'Analyze the source: research question, method, evidence, contribution, novelty, comparison baseline, limitations, and what is not established. Quote or point to supplied evidence where possible.'},{agentId:'coding_assistant',task:'Act as a rigorous method reviewer. Check causal claims, assumptions, reproducibility, data or model risks, alternative explanations, and what would falsify the main conclusion.'},{agentId:'research_assistant',task:'Deliver a Chinese research brief with: one-sentence judgment, evidence table, novelty assessment, limitations, relevance to the stated goal, and 3 prioritized next actions. Mark uncertainty plainly.'}]},
  meeting_assistant:{templateVersion:2,inputGuide:'Provide meeting purpose, attendees or roles, time box, background, and any notes or decisions already made.',outputContract:'Before-meeting agenda or after-meeting decision record with owners and dates.',qualityChecklist:['Every agenda item has a decision or output','Actions have owner and due-date status','Separates decisions from discussion notes'],steps:[{agentId:'daily_assistant',task:'Create a time-boxed agenda. For every item state context, desired decision, owner, preparation needed, and time allocation. Flag missing information that blocks a decision.'},{agentId:'research_assistant',task:'Review the agenda or notes for unresolved assumptions, evidence needed, risks, and questions that should be answered before committing.'},{agentId:'daily_assistant',task:'Deliver a Chinese meeting pack: objective, agenda, decision questions, note-taking table, and after-meeting action list with owner and deadline placeholders. If notes were supplied, also list confirmed decisions and open items.'}]},
  daily_report:{templateVersion:2,inputGuide:'Provide completed work, in-progress items, blockers, numbers or evidence, upcoming priorities, and audience.',outputContract:'A factual daily or weekly report with priorities, risks, and requests for support.',qualityChecklist:['Does not invent progress','Separates completed work from plan','Makes blockers and requests actionable'],steps:[{agentId:'daily_assistant',task:'Normalize the raw notes into completed, in progress, blocked, planned, and support-needed. Preserve dates, owners, and metrics exactly; mark missing evidence.'},{agentId:'research_assistant',task:'Review for risk, dependency, contradiction, and priority drift. Identify what needs escalation and what can wait.'},{agentId:'daily_assistant',task:'Deliver a concise Chinese daily or weekly report: executive summary, progress, evidence or metrics, blockers, risks, next priorities, and explicit asks. Do not overstate certainty.'}]},
  copywriting_designer:{templateVersion:2,inputGuide:'Provide channel, audience, objective, source facts, tone, constraints, and any call to action.',outputContract:'A channel-appropriate content package with angles, draft, and self-edit notes.',qualityChecklist:['Claims are supported by supplied material','Tone fits the channel and audience','Includes a specific call to action without clickbait'],steps:[{agentId:'research_assistant',task:'Extract credible audience insights, source-backed claims, proof points, risks, and claims that must not be made. Generate 3 differentiated content angles.'},{agentId:'daily_assistant',task:'Select the strongest angle and draft a Chinese content piece suited to the stated channel. Include title options, hook, structure, body, call to action, and optional visual suggestions.'},{agentId:'daily_assistant',task:'Self-edit for clarity, specificity, audience fit, and unsupported claims. Deliver final copy plus a short checklist of facts requiring user confirmation before publishing.'}]},
...
function buildStepPrompt({ workflow, step, sourceInput, previousOutputs }) {
  ...
  return [
    ...
    workflow.inputGuide ? `Input contract: ${workflow.inputGuide}` : '',
    workflow.outputContract ? `Final output contract: ${workflow.outputContract}` : '',
    (workflow.qualityChecklist || []).length ? `Quality checklist:\n${workflow.qualityChecklist.map((item) => `- ${item}`).join('\n')}` : '',
    `Source material:\n${sourceInput}`,
    prior ? `Previous agent outputs:\n${prior}` : "",
    "Return the best possible output for the current task. Do not invent facts or external actions. Check the applicable quality criteria before finalizing."
  ].filter(Boolean).join("\n\n");
}
...
async function handleApiInWorkspace(req, res, pathname) {
  try {
    ...
    if (req.method === "POST" && pathname === "/api/collaboration-pilot") {
      const result = await runCollaborationPilot(readDb());
      return sendJson(res, 200, result);
    }
